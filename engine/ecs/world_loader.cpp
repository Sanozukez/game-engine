...
Get();
            ...
            {
                Engine::Core::Log::Critical("Falha crítica ao carregar arquivo de cena (.mmap).");
                return false;
            }
            ...
            {
                Component::Transform &transform = world.getComponent<Component::Transform>(playerID);
                ...
                Engine::Core::Log::Info(std::format("Player Entity (ID {}) spawnada em ({}, {}, {}) com offset.", playerID, spawnPosition.x, spawnPosition.y, spawnPosition.z));
            }
            ...
            if (terrainNode)
            {
                ...
                world.addComponent<Component::Transform>(terrainEntity);

                world.addComponent<Component::Mesh>(terrainEntity, terrainAssetID);
                world.addComponent<Terrain>(terrainEntity);

                // Configuração da Posição
                Component::Transform &transform = world.getComponent<Component::Transform>(terrainEntity);
                transform.position = Engine::Math::Vec3(terrainNode->position[0], terrainNode->position[1], terrainNode->position[2]);

                Engine::Core::Log::Info("Terrain Entity BASE carregada como componente ECS!");
            }
            ...
            for (const auto &node : loaded_nodes)
            {
                ...
                if (node.type == EntityType::TYPE_STATIC_MESH)
                {
                    ...
                    {
                        Engine::Core::Log::Error(std::format("Falha ao obter modelo (ID: {}). Pulando instanciação.", asset_id));
                        continue;
                    }
                    ...
                    // NOVO E CRÍTICO: Adicionar Transform
                    world.addComponent<Component::Transform>(moduleEntity);

                    world.addComponent<Component::Mesh>(moduleEntity, asset_id);
                    ...
                    Component::Transform &transform = world.getComponent<Component::Transform>(moduleEntity);
            ...
            Engine::Core::Log::Info("World Loader: Carregamento de mapa concluído. Entities criadas.");
            return true;
        }
