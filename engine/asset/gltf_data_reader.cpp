...
std::unique_ptr<Mesh> GltfDataReader::loadPrimitive(
    ...
    const glm::mat4 &nodeTransform /* NEW */)
{
    ...
    {
        auto meshPtr = std::make_unique<Engine::Asset::Mesh>(
            std::move(finalVertices),
            std::move(indices),
            std::move(material),
            nodeTransform // NEW
        );
        Engine::Core::Log::Debug("GltfDataReader: Malha construída com nodeTransform.");
        return meshPtr;
    }
    return nullptr;
}