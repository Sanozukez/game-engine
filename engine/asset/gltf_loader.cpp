...
namespace Engine
{
    namespace Asset
    {
        ...
        std::unique_ptr<Model> GLTFLoader::loadGLTF(const std::string &filePath)
        {
            ...
            // --------------------------------------------------------------------------------
            // NOVO: 1. Processar Esqueletos e Keyframes (SRP MOVIDO PARA NOVO MÓDULO)
            // --------------------------------------------------------------------------------
            int rootNodeIndex = AnimationLoader::processAnimationData(data, *model);

            // 2. Processar a Hierarquia do Modelo (Continua no gltf_loader)
            if (data->nodes_count > 0)
            {
                // Lógica de processamento de nodos para draw (não-esquelético)
                // ...
            }
            ...
            for (cgltf_size scene_idx = 0; scene_idx < data->scenes_count; ++scene_idx)
            {
                ...
                for (cgltf_size node_idx = 0; node_idx < gltfScene->nodes_count; ++node_idx)
                {
                    ...
                    if (gltfNode->mesh)
                    { // Se o nó tem uma malha associada
